...
class PPOTrainer(Trainer):
    ...
    def __init__(self, cfg: TrainingConfig, actor: GPTActor, critic: GPTCritic,
        ...
        self.actor = self.orig_actor # self.actor = torch.compile(self.orig_actor)
        self.critic = self.orig_critic #self.critic = torch.compile(self.orig_critic)
        self.sft_model = self.orig_sft_model #self.sft_model = torch.compile(self.orig_sft_model)
        self.reward_model = self.orig_reward_model # self.reward_model = torch.compile(self.orig_reward_model)
    ...
    def fit(self):
        ...
        for epoch in range(self.total_epochs):
            for step, (prompt, input_masks, input_lengths) in enumerate(pbar := tqdm(self.train_dataloader)):
                ...
                '''
                if total_steps == 500 or total_steps == 12000 or (
                    total_steps != 0
...
class SFTTrainer(Trainer):
    ...
    def __init__(self, cfg: TrainingConfig, device, model: nn.Module,
        ...
        self.max_steps = len(train_dataset)*2
        self.eval_freq = 1
        self.save_freq = 2000
    ...
    def fit(self):
        ...
        opt_model = self.model #opt_model = torch.compile(self.model)
